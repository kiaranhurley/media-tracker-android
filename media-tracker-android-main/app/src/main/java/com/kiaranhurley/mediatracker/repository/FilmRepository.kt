...
class FilmRepository @Inject constructor(
    ...
    private fun parseReleaseDate(dateString: String?): Date {
        return try {
            if (dateString.isNullOrBlank()) {
                Date() // Default to current date if null or empty
            } else {
     ...
     */
    suspend fun searchFilmsFromApi(query: String): List<Film> {
        return try {
            val response = tmdbService.searchMovies(tmdbApiKey, query)
            if (response.isSuccessful && response.body() != null) {
                ...
                val films = movies.map { movie ->
                    Film(
                        ...
                        productionCompanies = null // Will be filled when getting details
                    )
                }
     ...
     */
    suspend fun getPopularFilmsFromApi(): List<Film> {
        return try {
            val response = tmdbService.getPopularMovies(tmdbApiKey)
            if (response.isSuccessful && response.body() != null) {
                val movies = response.body()!!.results
                val films = movies.map { movie ->
                    Film(
     ...
     */
    suspend fun getFilmDetailsFromApi(tmdbId: Int): Film? {
        return try {
            val response = tmdbService.getMovieDetails(tmdbId, tmdbApiKey)
            if (response.isSuccessful && response.body() != null) {
                ...
                val film = Film(
                    ...
                    productionCompanies = movie.productionCompanies?.joinToString(", ") { it.name }
                )
